...
public:
    Input<Buffer<InPixelType>>  input{"input", 3};
    Output<Buffer<InPixelType>> output{"output", 3};
    ...
    void generate() {
        ...
        input_copy(x, y, c) = cast<InPixelType>(clamped(x, y, c));
        hw_input(x, y, c) = cast<float>(input_copy(x, y, c));

        Func hw_output;
        hw_output(x, y, c) =
          cast<InPixelType>(max(
                hw_input(2*x, 2*y, c),
